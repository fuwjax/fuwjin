...
import org.fuwjin.milik.Constants;
import org.fuwjin.milik.mirrors.ClassMirrorNotFoundException;
...
public class TypeDesc {
  ...
  /**
   * Given two type descriptors, it returns an appropriate merge: 1) If they are
   * Array types, the result is a an array of the merged component types 2) If
   * they are ref types, it returns the least common super type. If one of them
   * is an interface, the result is D_OBJECT 3) All other types must match
   * exactly in order to not raise an error.
   */
    ...
    case '[':
      if (bc == '[') {
        try {
          ...
          // The component types are incompatible, but two disparate arrays
          // still
          // inherit from Object
          return D_OBJECT;
        }
  ...
  // System.out.println(mergeType("Lorg/fuwjin/milik/test/ex/ExC;",
