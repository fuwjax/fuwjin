...
public final class Fiber {
   ...
   * stateStack[iStack] if iStack is in the (0..stateStack.length-1) range, and
   * null otherwise. This is used by the generated code to avoid having to
   ...
   * which alternates between pausing and not pausing in successive invocations.
   * 
   * @param f
   */
   ...
   * nullify it in stateStack. This accomplishes the equivalent of: move up get
   * state() restore from state delete state()
   * 
  ...
  static void ds() {
    for (StackTraceElement ste : new Exception().getStackTrace()) {
      ...
      if (cl.startsWith("org.fuwjin.milik.Worker") || meth.equals("go") || meth.equals("ds"))
        continue;
   ...
   * returning or pausing. If, however, an exception is thrown, we lose track of
   * where we are in the hierarchy. We recalibrate iStack by creating a dummy
   * exception and comparing it to the stack depth of an exception taken
   * earlier. This is done in scheduler.getStackDepth(); A sample stack trace of
   * the dummy exception looks as follows
   * 
   * <pre>
   *   at org.fuwjin.milik.Fiber.upEx(Fiber.java:250)
   *   at org.fuwjin.milik.test.ex.ExCatch.normalCatch(ExCatch.java)
   *   at org.fuwjin.milik.test.ex.ExCatch.test(ExCatch.java)
   *   at org.fuwjin.milik.test.ex.ExCatch.execute(ExCatch.java)
   *   at org.fuwjin.milik.Task.runExecute(Task.java)
   *   at org.fuwjin.milik.WorkerThread.run(WorkerThread.java:11)
   * </pre>
   * 
  ...
  void togglePause() {
    ...
    if (curState == null) {
      ...
      assert curState == PAUSE_STATE : "togglePause: Expected PAUSE_STATE, instead got: iStack == " + iStack
          + ", state = " + curState;
