...
public class CompositeFunction implements Function {
   ...
   @Override
   public FunctionRenderer renderer(final boolean castArgs) {
      return new FunctionRenderer() {
         private final List<String> args = new ArrayList<String>();

         @Override
         public void addArg(final String rep) {
            args.add(rep);
         }
         ...
         public String render() {
            for(final FunctionTarget function: functions) {
               if(function.requiredArguments() == args.size()) {
                  return function.render(castArgs, args.toArray(new String[args.size()]));
               }
            }
            throw new RuntimeException("No method matched args: " + args);
         }
      };
   }
