...
public abstract class Task implements EventSubscriber {
  ...
  /**
   * The reason for pausing (duh) and performs the role of a await condition in
   * CCS. This object is responsible for resuming the task.
   * 
   * @see org.fuwjin.milik.PauseReason
   */
   ...
   * 
   * @see org.fuwjin.milik.ReentrantLock
   */
   ...
   * 
   * @return the number of stack frames above _runExecute(), not including this
   *         method
   */
  ...
  /**
   * Exit the task with a throwable indicating an error condition. The value is
   * conveyed through the exit mailslot (see informOnExit). All exceptions
  ...
  public static void errNotWoven() {
    throw new RuntimeException("Task has either not been woven or the classpath is incorrect");
  }

  public static void errNotWoven(Task t) {
    throw new RuntimeException("Task " + t.getClass() + " has either not been woven or the classpath is incorrect");
  }

  static class ArgState extends org.fuwjin.milik.State {
    Object mthd;
    Object obj;
    Object[] fargs;
  }
   ...
   * @param target
   *          : The object on which the method is invoked. Can be null if the
   *          method is static.
   * @param args
   ...
   */
  public static Object invoke(Method mthd, Object target, Object... args) throws Pausable, IllegalAccessException,
  ...
  // Given a method corresp. to "f(int)", return the equivalent woven method for
  // "f(int, org.fuwjin.milik.Fiber)"
  private static Method getWovenMethod(Method m) {
    Class<?>[] ptypes = m.getParameterTypes();
    if (!(ptypes.length > 0 && ptypes[ptypes.length - 1].getName().equals("org.fuwjin.milik.Fiber"))) {
      ...
      LOOP: for (Method wm : m.getDeclaringClass().getDeclaredMethods()) {
        if (wm != m && wm.getName().equals(m.getName())) {
          // names match. Check if the wm has the exact parameter types as m,
          // plus a fiber.
          Class<?>[] wptypes = wm.getParameterTypes();
          if (wptypes.length != ptypes.length + 1
              || !(wptypes[wptypes.length - 1].getName().equals("org.fuwjin.milik.Fiber")))
            continue LOOP;
  ...
  /**
   * @param millis
   *          to sleep. Like thread.sleep, except it doesn't throw an interrupt,
   *          and it doesn't hog the java thread.
   */
  public static void sleep(final long millis) throws Pausable {
    // create a temp mailbox, and wait on it.
    final Mailbox<Integer> sleepmb = new Mailbox<Integer>(1); // TODO: will need
    // a better
    // mechanism for
    // monitoring
    // later on.
  ...
  /**
   * Ask the current task to pause with a reason object, that is responsible for
   * resuming the task when the reason (for pausing) is not valid any more.
   * 
   * @param pauseReason
   *          the reason
   */
   ...
   * 
   * If the subclass has not been woven, it won't have an execute method of the
   * following form, and this method will be called instead.
   */
   ...
   * execute processing (in addition to calling the execute(fiber) method of the
   * task.
   */
  public void _runExecute(WorkerThread thread) throws NotPausable {
    ...
    try {
      ...
      assert (preferredResumeThread == null || preferredResumeThread == thread) : "Resumed " + id
          + " in incorrect thread. ";
      ...
      assert (pauseReason == null && isDone) || (pauseReason != null && !isDone) : "pauseReason:" + pauseReason
          + ",isDone =" + isDone;
    } catch (Throwable th) {
      th.printStackTrace();
      // Definitely done
