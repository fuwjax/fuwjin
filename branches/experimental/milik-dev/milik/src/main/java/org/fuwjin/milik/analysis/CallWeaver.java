 ...
 */
package org.fuwjin.milik.analysis;

import static org.fuwjin.milik.Constants.ALOAD_0;
import static org.fuwjin.milik.Constants.ASTORE_0;
import static org.fuwjin.milik.Constants.DLOAD_0;
import static org.fuwjin.milik.Constants.DSTORE_0;
import static org.fuwjin.milik.Constants.D_BOOLEAN;
import static org.fuwjin.milik.Constants.D_BYTE;
import static org.fuwjin.milik.Constants.D_CHAR;
import static org.fuwjin.milik.Constants.D_DOUBLE;
import static org.fuwjin.milik.Constants.D_FIBER;
import static org.fuwjin.milik.Constants.D_FLOAT;
import static org.fuwjin.milik.Constants.D_INT;
import static org.fuwjin.milik.Constants.D_LONG;
import static org.fuwjin.milik.Constants.D_NULL;
import static org.fuwjin.milik.Constants.D_OBJECT;
import static org.fuwjin.milik.Constants.D_SHORT;
import static org.fuwjin.milik.Constants.D_STATE;
import static org.fuwjin.milik.Constants.D_UNDEFINED;
import static org.fuwjin.milik.Constants.D_VOID;
import static org.fuwjin.milik.Constants.FIBER_CLASS;
import static org.fuwjin.milik.Constants.FLOAD_0;
import static org.fuwjin.milik.Constants.FSTORE_0;
import static org.fuwjin.milik.Constants.ILOAD_0;
import static org.fuwjin.milik.Constants.ISTORE_0;
import static org.fuwjin.milik.Constants.LLOAD_0;
import static org.fuwjin.milik.Constants.LSTORE_0;
import static org.fuwjin.milik.Constants.STATE_CLASS;
import static org.fuwjin.milik.analysis.VMType.TOBJECT;
import static org.fuwjin.milik.analysis.VMType.loadVar;
import static org.fuwjin.milik.analysis.VMType.storeVar;
import static org.fuwjin.milik.analysis.VMType.toVmType;
...
public class CallWeaver {
  ...
  /**
   * The canoncial name of the state class responsible for storing the state (@see
   * {@link State})
   */
  ...
  /**
   * The basic block's frame tells us the number of parameters in the stack and
   * which local variables are needed later on.
   * 
   * If the method is pausable, we'll need the bottom part of the stack and the
   * object reference from the top part of the stack to be able to resume it. We
   * don't need to worry about the arguments, because they will be saved (if
   * needed) in the _called_ method's state.
   * 
   * The "this" arg (var0) is given special treatment. It is always saved in all
   * states, so it doesn't count as "data".
   */
  private void assignRegisters() {
    ...
    /*
     * Create ValInfos for each Value that needs to be saved (all live-in vars
     * (except var 0, if not static) and all stack bottom vars count, except if
     * they are duplicates of earlier ones or are constants which can be
     * reproduced in bytecode itself.
     * 
    ...
    /*
     * All stack values at the bottom (those not consumed by the called method)
     * will have to be saved, if they are are not already accounted for or they
     * are constants.
     */
  ...
  /**
   * Before we make the target call, we need to call fiber.down(), to update it
   * on the depth of the stack. genPostCall subsequently arranges to call
   ...
   * called method yielded, or it returned normally. Orthogonally, we have saved
   * state or not. fiber.up() returns the combined status
   * 
   * ? *
   * 
   * <pre>
  ...
  /**
   * Code for the case where we are yielding, and we have state built up from a
   * previous call. There's nothing meaningful to do except keep the verifier
   * happy. Pop the bottom stack, then return a dummy return value (if this
   * method -- note: not the called method -- returns a value)
   * 
   * @param mv
   */
  ...
  /**
   * Yielding, but state hasn't been captured yet. We create a state object and
   * save each object in valInfoList in its corresponding field.
   * 
   * Note that we save each stack item into a scratch register before loading it
   * into a field. The reason is that we need to get the State ref under the
   * stack item before we can do a putfield. The alternative is to load the
   * State item, then do a swap or a dup_x2;pop (depending on the value's
  ...
  private void genSave(MethodVisitor mv, Label saveLabel) {
    ...
    /*
     * Instantiate state class. Call one of new xxxState(this, pc, fiber), or
     * new xxxState(pc, fiber) depending whether this method is static or not.
     * Note that are not referring to the called method.
     * 
     * pc, "the program counter" is merely the index of the call weaver in the
     * method weaver's list. This allows us to do a switch in the method's
     * entry.
     */
    ...
    for (; i >= 0; i--) {
      ...
      if (vi == null) {
        ...
        /**
         * xstore <scratchVar> ; send stack elem to some local var aload
         * <stateVar> ; load state xload <scratchVar> ; get stack elem back
         * putfield ... ; state.fx =
         */
  ...
  /**
   * Not yielding (resuming normally), but have stored state. We need to restore
   * from state before resuming. This is slightly more work than saving state,
   * because we have to restore constants and duplicates too. <b>
   * 
   * Note that the return value (if any) has a real value, which is why it needs
   * to be saved away before we can get access to the bottom elements to pop
   * them out.
   * 
   * <pre>
    ...
    for (; i < len; i++) {
      ...
      if (v.isConstant()) {
        ...
        if (vi.var == i) {
          ...
          // It is a duplicate of another var. No need to load this var from
          // stack
          assert vi.var < i;
          loadVar(mv, vi.vmt, vi.var);
        }
  ...
  private String createStateClass() {
    return valInfoList.size() == 0 ? STATE_CLASS : methodWeaver.createStateClass(valInfoList);
  }
...
class VMType {
  ...
  static final int[] constInsn = { ACONST_NULL, ICONST_0, LCONST_0, DCONST_0, FCONST_0 };

  static final int[] loadInsn = { ALOAD, ILOAD, LLOAD, DLOAD, FLOAD };

  static final int[] retInsn = { ARETURN, IRETURN, LRETURN, DRETURN, FRETURN };

  static final int[] ldInsn = { ALOAD_0, ILOAD_0, LLOAD_0, DLOAD_0, FLOAD_0 };

  static final int[] stInsn = { ASTORE_0, ISTORE_0, LSTORE_0, DSTORE_0, FSTORE_0 };

  static final int[] storeInsn = { ASTORE, ISTORE, LSTORE, DSTORE, FSTORE };

  static final String[] fieldDesc = { D_OBJECT, D_INT, D_LONG, D_DOUBLE, D_FLOAT };
