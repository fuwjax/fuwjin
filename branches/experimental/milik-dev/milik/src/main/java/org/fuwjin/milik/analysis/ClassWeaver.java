...
import static org.fuwjin.milik.Constants.WOVEN_FIELD;
...
public class ClassWeaver {
  ...
  private void accept(final ClassVisitor cv) {
    ...
    /*
     * Mark this class as "processed" by adding a dummy field, so that we don't
     * weave an already woven file
     */
   ...
   * weaving. Examples are Task.yield and Task.sleep
   */
  ...
  /**
   * Create a custom class (structure) to hold the state. The name of the state
   * reflects the numbers of the various VMtypes in valInfoList. class SO2IJ3
   * reflects a class that stores two Objects one Integer and 3 longs.
   * 
   * <pre>
   * class SO2IJ3 extends State {
   *   public Object f1, f2;
   *   public int f3;
   ...
   * </pre>
   * 
   * If there's no data to store, we use the State class directly to store the
   * basic amount of data necessary to restore the stack.
   */
  ...
  String createStateClass(ValInfoList valInfoList) {
    ...
    ClassWriter cw = new ClassWriter(false);
    cw.visit(V1_1, ACC_PUBLIC | ACC_FINAL, className, null, "org/fuwjin/milik/State", null);
