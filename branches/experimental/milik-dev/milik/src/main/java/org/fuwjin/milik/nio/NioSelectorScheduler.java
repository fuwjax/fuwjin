...
import org.fuwjin.milik.RingQueue;
...
public class NioSelectorScheduler extends Scheduler {
  ...
  public Selector sel;
  /*
   * The thread in which the selector runs. THe NioSelectorScheduler only runs
   * one thread, unlike typical schedulers that manage a pool of threads.
   */
  ...
  static class SelectorThread extends Thread {
    ...
    public void run() {
      ...
      RingQueue<Task> runnables = new RingQueue<Task>(100); // to swap with
                                                            // scheduler
      while (true) {
        int n;
        try {
        ...
        while (runnables.size() > 0) {
          ...
          t._runExecute(null);
          // If task calls Task.yield, it would have added itself to scheduler
          // already.
